...
- **Single source of truth is the session corpus — there is NO `llm/stream` hook.** v3 removed the waterfall live capture: `sessionQuery` is live-preferred (in-memory active sessions plus persisted logs), so it is complete and current on its own. Counting from both paths double-counted every call the moment a backfill re-read its events (observed ~2× inflation). `backfill()` is an **idempotent full rebuild**: it folds every in-window `assistant/message` usage event into a FRESH bucket map (`foldInto`) and swaps it into `daily` atomically, so repeated runs never double-count and stale buckets self-heal. Runs are **coalesced** (`backfilling` flag) and **throttled to one per 20 s** (`lastBackfillAt`); on failure the previous buckets stay served. The snapshot route kicks off the rebuild **fire-and-forget** — it never awaits the fold (a full fold costs ~1 s on a ~20-session corpus, and the page must get the current buckets immediately); the startup backfill runs 3 s after boot. `backfilledUntil` is display-only metadata ("历史回填至").
- `sessionQuery` is fetched via `ctx.get('sessionQuery')` and may be `undefined` — guard it. `listSessions()` returns records with `header` (`id`, `createdAt`, `delegationDepth`); `readSession(id)` returns `log.events` (`type: 'assistant/message'`, `data.usage`, `data.message.source` → `{ provider, model }`, `time` — ms epoch, always numeric for usage events). Sessions are not woken; zstd decoding is internal. Only `delegationDepth === 0` sessions count as conversations (usage counts ALL sessions — subagents are real model calls). Model attribution for events comes from `message.source` (`sourceModelKey`, fallback `"unknown"`).
...
- Schema migrations: on load, `saved.version !== SAVED_VERSION` discards the old daily buckets; the startup rebuild restores the whole window from the corpus (v1/v2 → v3 widened the retention window to 181 days). The pre-rename path `$DSH_HOME/plugins/token-usage/data.json` (plugin id was `token-usage`) is still renamed to the new path on first load — keep both. The v3 accounting fix needs no version bump: the idempotent rebuild replaces inflated buckets on the next run.
