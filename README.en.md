...
A [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) plugin that puts a **"今日用量 / today's usage" card at the sidebar foot** (next to the Settings gear) showing today's token total in real time (e.g. "今日用量：8888万"); **clicking the card pops the full stats window** — today / 7-day / 30-day token totals, a 30-day per-model stacked usage chart, a 30-day per-model usage ranking (fixed window), and conversation counts. No settings page involved: one click and you're in.

![Today usage card](screenshot.png)

## Features

- 🚀 **One-click access** — the sidebar "今日用量" card shows today's token total live on one line ("今日用量：8888万"; refreshes every 30 s; the 56 px rail keeps only the compact value); clicking opens the stats window (close with Esc / ✕ / clicking the mask; background scroll is locked while open).
...
**Restart DSH**: the "今日用量 / today's usage" card appears at the sidebar foot (next to the Settings gear); click it to pop the stats window.
...
│  • webServer.register('/token-monitor/today') ← light today read     │
│      (no rebuild; polled by the sidebar card every 30 s)             │
...
└─────────────────────────────────────────────────────────────────────┘
                          │ fetch('/token-monitor/today' | '/token-monitor/snapshot')
                          ▼
┌────────────────────────── Client (browser) ────────────────────────┐
│ client/bundle.js — hand-built web bundle following the             │
│ client-modules protocol (window.__ModuleLoader__.load)             │
│  • slots.inject('sidebar.footer.action') → "今日用量" card           │
│      (click pops a fixed modal hosting the full dashboard; no      │
│      settings.section registration anymore)                        │
│  • metric cards + conversation cards + 30-day stacked chart +      │
│    model usage ranking                                             │
│  • card polls /token-monitor/today every 30 s; popup fetches the   │
│    snapshot on open and polls every 30 s, refresh / backfill       │
│    buttons                                                         │
└─────────────────────────────────────────────────────────────────────┘
```
