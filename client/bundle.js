 ...
 * the full content width. Visual language follows the DSH light theme:
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ".tu-root { display:flex; flex-direction:column; gap:16px; padding:6px 2px; font-family: inherit; }",
      ".tu-head { display:flex; align-items:center; justify-content:space-between; gap:8px; flex-wrap:wrap; }",
      ".tu-title { font-size:16px; font-weight:600; margin:0; color:var(--ds-text, #1a1a1a); }",
      ".tu-btn { border:1px solid var(--ds-border, #d5d5d5); border-radius:8px; background:transparent; color:var(--ds-text, #333); padding:6px 14px; font-size:13px; cursor:pointer; transition:background .15s ease; }",
      ".tu-btn:hover:not(:disabled) { background:rgba(127,127,127,.08); }",
      ".tu-btn:disabled { opacity:.5; cursor:default; }",
      ".tu-meta { font-size:12px; opacity:.6; }",
      ".tu-cards { display:grid; grid-template-columns:repeat(3,1fr); gap:12px; }",
      ".tu-card { border:1px solid var(--ds-border, #e5e5e5); border-radius:10px; padding:14px 16px; background:var(--ds-surface, #fff); }",
      ".tu-card-label { font-size:12px; opacity:.6; }",
      ".tu-card-value { font-size:26px; font-weight:700; margin-top:4px; letter-spacing:-.02em; color:var(--ds-text, #1a1a1a); line-height:1.15; }",
      ".tu-card-sub { font-size:11px; opacity:.6; margin-top:4px; }",
      ".tu-graph { display:flex; flex-direction:column; gap:6px; }",
      ".tu-graph-months { display:grid; gap:3px; margin-left:26px; }",
      ".tu-month { font-size:10px; opacity:.6; white-space:nowrap; overflow:visible; }",
      ".tu-graph-body { display:flex; gap:6px; }",
      ".tu-graph-labels { display:grid; grid-template-rows:repeat(7,1fr); gap:3px; width:20px; font-size:10px; opacity:.55; }",
      ...
      ".tu-legend { display:flex; align-items:center; gap:4px; font-size:10px; opacity:.6; }",
      ".tu-swatch { width:10px; height:10px; border-radius:2px; }",
      ".tu-err { border:1px solid rgba(239,68,68,.5); background:rgba(239,68,68,.08); border-radius:8px; padding:8px 12px; font-size:12px; color:#ef4444; }"
    ].join("\n")
